...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Đoạn này lấy activeBoard từ redux để mục đích là lấy toàn bộ thông tin những thành viên của cái Board thông qua field: FE_allUsers
  const board = useSelector(selectCurrentActiveBoard);

  /**
   * Thành viên trong card sẽ phải là tập con của thành viên trong board
   * Vì thế dựa vào mảng board.Fe_allUsers và card.memberIds rồi chúng ta tạo ra một mảng FE_CardMembers chứa đủ thông tin của User để hiển thị ra ngoài giao diện bởi mặc định
    trong card chỉ lưu đám Id của User thôi (memberIds)
   */
  ...
  const handleUpdateCardMembers = (user) => {
    // Tạo một biến incomingMemberInfo để gửi cho BE, với 2 thông tin chính là userId và action là
    // xóa khỏi card (REMOVE) hoặc thêm vào card (ADD)
